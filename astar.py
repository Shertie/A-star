    ...
    def visualize(self, grid, closed_set, open_list_positions, current_pos, final_path=None):
        ...
        for r in range(grid.shape[0]):
            for c in range(grid.shape[1]):
                ...
                pygame.draw.rect(self.screen, self.colors['grid_line'], rect, 1)  # Lines for grid

        # Closed set (light grey)
        for pos in closed_set:
            if pos != self.start_pos and pos != self.goal_pos:  # Don't overwrite start/goal
                pygame.draw.rect(self.screen, self.colors['closed_set'],
                                 (pos[1] * self.cell_size + 1, pos[0] * self.cell_size + 1, self.cell_size - 2,
                                  self.cell_size - 2))
        ...
        # Current node being processed (blue)
        if current_pos and current_pos != self.start_pos and current_pos != self.goal_pos:
            pygame.draw.rect(self.screen, self.colors['current'], (
                current_pos[1] * self.cell_size + 1, current_pos[0] * self.cell_size + 1, self.cell_size - 2,
                self.cell_size - 2))
        ...
        # Start is green, goal is red
        start_rect = pygame.Rect(self.start_pos[1] * self.cell_size, self.start_pos[0] * self.cell_size, self.cell_size,
                                 self.cell_size)
